...
- **티커 조회**: `AAPL`, `MSFT`, `005930.KS`(삼성전자) 등 yfinance가 지원하는 티커를 입력해 조회
- **7가지 밸류에이션 비율** 시각화
  | 지표 | 의미 | 계산식 |
  |------|------|--------|
  | PER | 주가수익비율 | 시가총액 / 순이익 |
...
웹 서버 없이 특정 티커의 결과를 터미널에서 바로 볼 수 있습니다.
...
{
  "ticker": "AAPL",
  "dates": ["2022-09-24", "2023-09-30", "2024-09-28"],
  "metrics": {
    "PER": [24.5, 29.1, 33.2],
    "PBR": [48.1, 45.0, 51.3],
    "PSR": [6.1, 7.0, 8.2],
    "PEG": [1.8, null, 2.1],
    "EV_EBITDA": [19.0, 21.5, 24.1],
    "DividendYield": [0.6, 0.5, 0.4],
    "FCFYield": [4.1, 3.8, 3.2]
  },
...
- 티커가 잘못됐거나 데이터가 부족하면 `400`과 `{"error": "..."}`를 반환합니다.
...
├── db.py               # SQLite 캐시 레이어 (원본 재무 수치 저장)
