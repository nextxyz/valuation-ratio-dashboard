...
- **주가는 배당 미조정 실제 종가**를 사용합니다(`auto_adjust=False`). yfinance 기본값은 배당까지 소급 차감해서 과거 주가가 실제보다 낮게(고배당주는 -20%대) 나오는데, 그러면 모든 종목이 "과거엔 싸다가 점점 비싸졌다"는 없는 추세를 보입니다. 분할은 소급 조정되며 Yahoo의 주식수도 분할 기준으로 재작성되므로 정합적입니다.
